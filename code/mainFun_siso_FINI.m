...
% 生成数据datain，导频帧
rng('default');
dain=randi([0 1],modDim.DataInputSize(1)./2,1);% 生成数据帧(加了卷积编码数据长度只有原来的一半)

%卷积编码
hConEnc = comm.ConvolutionalEncoder;
hDec = comm.ViterbiDecoder('InputFormat','Hard');
hError = comm.ErrorRate('ComputationDelay',3,'ReceiveDelay', modDim.DataInputSize(1)./2+4);

dain_JUAN = step(hConEnc, dain);
%交织
interleaver = comm.MatrixInterleaver('NumRows',4,'NumColumns', length(dain_JUAN)/4);
deinterleaver = comm.MatrixDeinterleaver('NumRows',4,'NumColumns', length(dain_JUAN)/4);
...
datain_ALL = awgn(after_Ray,Snr,'measured');%%%%%%%%aafterfm2:通过Rayleigh信道，aafterfm:不通过Rayleigh信道
...
% errors = step(hError, data, receivedBits);
        errVec = hError(data_FINI,dain);   % data_FINI,dain;
%         ber=[ber;errVec(1)];
BER=errVec(1);
