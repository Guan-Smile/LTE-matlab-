function [mse,f_averge,BER]=mainFun_mimo_fini(Snr,stp,chan_dis)
...
global K
snr=Snr;
% chan_dis=1;%%%%%%%%%%%1:Rayleigh+awgm  2:LTEmimo
MSE=[];
%% 1.1构造非ofdm数据帧包
% 设置ofdm调制解调模块
...
fD=0.01;%Hz
% DelayVec=[0,3e-6];%s
DelayVec=[0,30,150,310,370,710,1090,1730,2510]*1e-9;%s
% GainVec=[0,-100];%dB
GainVec=[0.0,-1.5,-1.4,-3.6,-0.6,-9.1,-7.0,-12.0,-16.9];%dB
% fs=Rb/N*N;%采样频率
% rng('default');
chan11=rayleighchan(1/fs,fD,DelayVec,GainVec);chan11.StoreHistory=1;
chan12=rayleighchan(1/fs,fD,DelayVec,GainVec);chan12.StoreHistory=1;
chan21=rayleighchan(1/fs,fD,DelayVec,GainVec);chan21.StoreHistory=1;
chan22=rayleighchan(1/fs,fD,DelayVec,GainVec);chan22.StoreHistory=1;
y1=filter(chan11,datain_ALL)+filter(chan21,datain_ALL2);
y2=filter(chan12,datain_ALL)+filter(chan22,datain_ALL2);
...
[LTEOut,LTEPathGains] = lteChan(channelInput);%%%%%%%%%%%lteChan
y1=awgn(LTEOut(:,1),snr,'measured');
y2=awgn(LTEOut(:,2),snr,'measured');
...
Data_AT_ALL(:,lh)=Data_atertFrelock;%%%%%%%%%%%%%%%Data_atertimelock///Data_atertFrelock
